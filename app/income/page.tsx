...
export default function IncomeList() {
  ...
  const uniqueCategories = useMemo(() => {
    return Array.from(new Set(incomes.map(e => e.category).filter(Boolean)));
  }, [incomes]);

  const uniqueCurrencies = useMemo(() => {
    return Array.from(new Set(incomes.map(e => e.currency).filter(Boolean)));
  }, [incomes]);
  ...
  const filteredIncomes = incomes.filter(inc => {
    ...
    const matchesCurrency = !currencyFilter || inc.currency === currencyFilter;
            ...
            <div className="space-y-6">
              ...
              {income.paymentMethod && (
                <div>
                  ...
                  <p className="text-sm font-semibold text-zinc-900 dark:text-white">
                    {income.paymentMethod === 'cash' ? (t('cash') || 'Cash') : (t('bankTransfer') || 'Bank Transfer')}
                  </p>
                </div>
              )}
              ...
              {income.notes && (
                <div>
                  ...
                  <p className="text-sm font-semibold text-zinc-900 dark:text-white leading-relaxed whitespace-pre-wrap">{income.notes}</p>
                </div>
              )}
